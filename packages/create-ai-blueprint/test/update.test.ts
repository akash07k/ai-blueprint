...
test("parseArgs supports install and update modes", () => {
  ...
  assert.throws(() => parseArgs(["--both"]), /Unknown option/);
});

test("adapter modes copy the expected files", () => {
  assert.deepEqual(
...
test("legacy detection identifies a Copilot install without adding Codex", async (t) => {
  const workspace = await createWorkspace(t);
  const templateRoot = path.join(workspace, "template");
  const targetDir = path.join(workspace, "target");
  const files = {
